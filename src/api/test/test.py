...
def test_count_movies():
    r = requests.get(
        url='http://{address}:{port}/movies/count'.format(address=api_address, port=api_port)
    )
...
def test_search_movies():
    r = requests.get(
        url='http://{address}:{port}/movies/search'.format(address=api_address, port=api_port),
        params={"query": "war"}
    )
